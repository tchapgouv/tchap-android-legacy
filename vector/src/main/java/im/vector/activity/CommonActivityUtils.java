        ...
        Matrix.getInstance(context).clearSessions(context, true, new SimpleApiCallback<Void>() {
            ...
            public void onSuccess(Void info) {
                ...
                if (goToLoginPage) {
                    ...
                    // go to login page
                    Intent intent = new Intent(activeContext, TchapLoginActivity.class);
                    intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
        ...
        session.getCrypto().exportRoomKeys(password, new SimpleApiCallback<byte[]>(callback) {
            ...
            public void onSuccess(byte[] bytesArray) {
                try {
                    ...
                    String url = session.getMediaCache().saveMedia(stream, "tchap-" + System.currentTimeMillis() + ".txt", "text/plain");
                    stream.close();

                    saveMediaIntoDownloads(appContext,
                            new File(Uri.parse(url).getPath()), "tchap-keys.txt", "text/plain", new SimpleApiCallback<String>(callback) {
                                @Override
